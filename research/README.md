...
- Видно, код транслируется без значительных оптимизаций. Массивы и циклы по 4 остались в ассемблерном коде. Производительность ~ `2 fps`
```c++
for (int i = 0; i < 4; i++) {
...
- При `-O2` компилятор свел работу с массивами и циклами по 4 к работе с мультимедийными регистрами xmm, что способствовало повешению производительности (до `40 fps`)
```c++
for (int i = 0; i < 4; i++) {
    x_arr4[i] = x2_arr4[i] - y2_arr4[i] + x0_arr4[i];
    y_arr4[i] = 2 * xy_arr4[i] + y0_arr4[i];
...
- При `-Ofast` к моему удивлению производительность снизилась. Это связано с тем, что компилятор решил отказаться от мультимедийных регистров в пользу развертывания цикла и выполнения инструкций в приятной конвейеру независимой по данным последовательности инструкций. Производительность ~ `12 fps`
...
```
O2_mandelbrot_set.exe 'array optimization'
: 41.88215630434783

O2_mandelbrot_set.exe 'without optimization'
: 8.532242857142858

O2_mandelbrot_set.exe 'intrinsic optimization'
: 58.56542856060608

O2_mandelbrot_set.exe 'intrinsic unroll 2 optimization'
: 97.90259115044252

Ofast_mandelbrot_set.exe 'array optimization'
: 12.069848

Ofast_mandelbrot_set.exe 'without optimization'
: 8.181082333333334

Ofast_mandelbrot_set.exe 'intrinsic optimization'
: 51.84438386363637

Ofast_mandelbrot_set.exe 'intrinsic unroll 2 optimization'
: 97.23500243697478

Og_mandelbrot_set.exe 'array optimization'
: 2.450440625

Og_mandelbrot_set.exe 'without optimization'
: 7.853005

Og_mandelbrot_set.exe 'intrinsic optimization'
: 54.49733524590164

Og_mandelbrot_set.exe 'intrinsic unroll 2 optimization'
: 87.53273614999999
